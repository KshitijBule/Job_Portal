...
const PostJob = () => {
  ...
  const handlePost = () => {
    ...
    postJob({ ...form.getValues(), id, postedBy: user.id, jobStatus: "OPEN" }).then((res) => {
  ...
  return (
    <div className="w-4/5 mx-auto max-sm:w-full max-sm:px-4">
      <div className="text-2xl font-semibold mt-4 mb-5">Post a Job</div>

      <div className="flex flex-col gap-6">

        {/* Row 1: Job Title + Company */}
        <div className="flex gap-10 [&>*]:w-1/2 max-sm:flex-col max-sm:gap-4 max-sm:[&>*]:w-full">
          <SelectInput form={form} name="jobTitle" {...select[0]} />
          <SelectInput form={form} name="company" {...select[1]} />
        </div>

        {/* Row 2: Experience + Job Type */}
        <div className="flex gap-10 [&>*]:w-1/2 max-sm:flex-col max-sm:gap-4 max-sm:[&>*]:w-full">
        ...
        {/* Skills */}
        <TagsInput
          {...form.getInputProps('skillRequired')}
          withAsterisk
          label="Skills"
        ...
        {/* Buttons */}
        <div className="flex gap-3 max-sm:flex-col pb-4">
